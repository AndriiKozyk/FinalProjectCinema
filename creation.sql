...
CREATE TABLE IF NOT EXISTS `cinema`.`genre` (
  ...
  `genre_ua` VARCHAR(45) NOT NULL,
  PRIMARY KEY (`id`),
  UNIQUE INDEX `genre_en_UNIQUE` (`genre_en` ASC),
  UNIQUE INDEX `genre_ua_UNIQUE` (`genre_ua` ASC))
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `cinema`.`type` (
  ...
  `type_ua` ENUM('Стандарт', 'Люкс', 'Супер люкс') NOT NULL,
  `price` DECIMAL NOT NULL,
  PRIMARY KEY (`id`))
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `cinema`.`status` (
  ...
  `status_ua` ENUM('Доступна', 'Немає вільних місць', 'Скасовано') NOT NULL,
  PRIMARY KEY (`id`))
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `cinema`.`film_status` (
  ...
  `status_ua` ENUM("запропонований", "на голосуванні", "підтверджений", "відхилений") NOT NULL,
  PRIMARY KEY (`id`),
  UNIQUE INDEX `status_en_UNIQUE` (`status_en` ASC),
  UNIQUE INDEX `status_ua_UNIQUE` (`status_ua` ASC))
ENGINE = InnoDB;
...
insert into status (status_en, status_ua) values ('Available', 'Доступна');
insert into status (status_en, status_ua) values ('No places', 'Немає вільних місць');
insert into status (status_en, status_ua) values ('Canceled', 'Скасовано');

insert into type (type_en, type_ua, price) values ('Standard', 'Стандарт', 10);
insert into type (type_en, type_ua, price) values ('Lux', 'Люкс', 15);
insert into type (type_en, type_ua, price) values ('Super lux', 'Супер люкс', 20);
...
insert into genre (genre_en, genre_ua) values ('Comedy', 'Комедія');
insert into genre (genre_en, genre_ua) values ('Action', 'Бойовик');
insert into genre (genre_en, genre_ua) values ('Horror', 'Фільм жахів');
insert into genre (genre_en, genre_ua) values ('Detective', 'Детектив');

insert into film_status (status_en, status_ua) values ('suggestion', 'запропонований');
insert into film_status (status_en, status_ua) values ('voting', 'на голосуванні');
insert into film_status (status_en, status_ua) values ('approved', 'підтверджений');
insert into film_status (status_en, status_ua) values ('rejected', 'відхилений');

insert into account_details (first_name_en, first_name_ua, last_name_en, last_name_ua, email, phone) 
	values ('Adminname', 'Ім\'яадміна', 'Adminlastname', 'Прізвищеадміна', 'admin@admin.com', '0501236547');
...
insert into account_details (first_name_en, first_name_ua, last_name_en, last_name_ua, email, phone) 
	values ('Username', 'Ім\'яюзера', 'Userlastname', 'Прізвищеюзера', 'user@user.com', '0501236549');
insert into account (login, password, role_id, account_details_id) 
